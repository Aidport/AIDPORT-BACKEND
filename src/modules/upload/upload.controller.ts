...
/**
 * Every account role persists delivery URLs to Mongo (unless `attachTo=none`):
...
export class UploadController {
  ...
  @ApiOperation({
    ...
    description:
      'Multipart field name: `file`. Max 10MB. **Agents** → `agentProfile.documentUrls`. **Shippers** → `shipperFileUrls`. **Admins** → `adminFileUrls`. All skipped if `attachTo=none`.',
  })
  @ApiQuery({
    name: 'attachTo',
    required: false,
